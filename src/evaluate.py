...
class Video:
    ...
    def split_frame(self, frame):
        if self.stack_type == "vertical":
            ...
            print("Error: unrecognized stack type `{}`!".format(self.stack_type))
            exit()
            ...
            if is_visible_in_both_stereo:
                if not is_difficult:
                    if v.is_bbox_inside_image(bbox1_gt, bbox2_gt):
                        # TODO add differentiation to baseline tracker
                        if tracker_type in config["baseline_tracker"]:
                            # print(
                            #     f"-------Loading baseline_tracker {tracker_type} -----------"
                            # )
                            t = Baseline_Tracker(
                                im1, im2, bbox1_gt, bbox2_gt, tracker_type=tracker_type
                            )
                        elif tracker_type=='fm_csrt_tracker':
                            t = FM_CSRT_Tracker(
                                im1, im2, bbox1_gt, bbox2_gt, config=config
                            )  # Initialise
...
def calculate_results(config, tracker_type, valid_or_test, is_visualization_off):
...
def evaluate_method(tracker_type, config, is_visualization_off):
    calculate_results(config, tracker_type, "validation", is_visualization_off)
    calculate_results(config, tracker_type, "test", is_visualization_off)